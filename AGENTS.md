...
### Start Details
| Field | Type | Contains User Content? | Description |
|-------|------|----------------------|-------------|
| task | string | ⚠️ **Yes** | The full task prompt provided to the agent |
| model | string | No | The LLM model used (e.g., `gpt-4o`, `claude-3-5-sonnet`) |
| model_provider | string | No | The provider (OpenAI, Anthropic, Google, Groq, etc.) |
| max_steps | int | No | Maximum steps configured for the agent |
| max_actions_per_step | int | No | Maximum actions per step configured |
| use_vision | bool \| Literal['auto'] | No | Whether vision/screenshots are enabled |
| version | string | No | Browser Use library version |
| source | string | No | How the library was invoked (SDK, CLI, MCP, etc.) |
| cdp_url | string \| None | No | Whether a custom CDP endpoint was used |
| agent_type | string \| None | No | The agent type used if specified |

### Step Details
| Field | Type | Contains User Content? | Description |
|-------|------|----------------------|-------------|
| action_history | sequence | ⚠️ **Yes** | All actions taken at each step, including text typed into form fields and pages |
| urls_visited | sequence | ⚠️ **Yes** | Every URL the agent navigated to during execution |
| action_errors | sequence | ⚠️ **Possible** | Error messages from failed actions (may contain context or page data) |

### End Details
| Field | Type | Contains User Content? | Description |
|-------|------|----------------------|-------------|
| steps | int | No | Total steps taken to complete the task |
| total_input_tokens | int | No | Total input tokens used across all LLM calls |
| total_output_tokens | int | No | Total output tokens generated by the LLM |
| prompt_cached_tokens | int | No | Tokens retrieved from prompt cache (if applicable) |
| total_tokens | int | No | Total tokens used (input + output + cached) |
| total_duration_seconds | float | No | Total time taken to complete the task |
| success | bool \| None | No | Whether the task completed successfully |
| final_result_response | string \| None | ⚠️ **Yes** | The final response/data extracted from the page by the agent |
| error_message | string \| None | ⚠️ **Possible** | Error message if task failed (may contain context or page data) |

### Judge Details (Optional)
| Field | Type | Contains User Content? | Description |
|-------|------|----------------------|-------------|
| judge_verdict | bool \| None | No | Whether a judge model approved the result |
| judge_reasoning | string \| None | ⚠️ **Possible** | Judge's reasoning (may reference task context) |
| judge_failure_reason | string \| None | ⚠️ **Possible** | Reason why judge rejected result |
| judge_reached_captcha | bool \| None | No | Whether a CAPTCHA was encountered |
| judge_impossible_task | bool \| None | No | Whether task was deemed impossible to complete |

### System Details
| Field | Type | Description |
|-------|------|-------------|
| is_docker | bool | Whether Browser Use is running in a Docker container |
