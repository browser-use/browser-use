...
class BrowserProfile(BrowserConnectArgs, BrowserLaunchPersistentContextArgs, BrowserLaunchArgs, BrowserNewContextArgs):
	...
	# Tracks whether user_data_dir was auto-generated by browser-use (True) or
	# explicitly supplied by the user (False). Used to conditionally add
	# --use-mock-keychain for managed profiles only (see #5415).
	_is_managed_browser_profile: bool = PrivateAttr(default=False)
	...
	def model_post_init(self, __context: Any) -> None:
		...
		# Detect managed profile before _copy_profile() may change user_data_dir.
		# A managed profile is one where user_data_dir was auto-generated by
		# browser-use (prefix 'browser-use-user-data-dir-'), not user-supplied.
		# user_data_dir=None also counts as managed (will get a temp dir later).
		if self.user_data_dir is None:
			self._is_managed_browser_profile = True
		else:
			self._is_managed_browser_profile = Path(self.user_data_dir).name.startswith('browser-use-user-data-dir-')
	...
	def get_args(self) -> list[str]:
		...
		pre_conversion_args = [
			...
			*(CHROME_DETERMINISTIC_RENDERING_ARGS if self.deterministic_rendering else []),
			# For browser-use-managed temporary profiles, use mock keychain to avoid
			# OS keychain prompts/hangs. User-supplied profiles keep real keychain
			# access for cookie/password decryption. See #5415.
			*(['--use-mock-keychain', '--password-store=basic'] if self._is_managed_browser_profile else []),
			*(
