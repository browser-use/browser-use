...
class BrowserContext:
	...
	async def click_element_node(
		...
		try:
			...
			if not element_handle:
				return ClickResult(status=ClickStatus.ERROR, message='Unable to locate element handle')
	...
	async def handle_click(self, page: Page, element: ElementHandle) -> ClickResult:
		...
		# Wait for events with timeout
			...
			timeout=max(self.config.click_config.timeouts['navigation'], self.config.click_config.timeouts['download']),
		...
		download_path = None
		if download_task in done and download_task.exception() is None and self.config.save_downloads_path:
			try:
				download_result = download_task.result()
				filename = await self._get_unique_filename(self.config.save_downloads_path, download_result.suggested_filename)
		...
		if download_success:
			return ClickResult(status=ClickStatus.DOWNLOAD_SUCCESS, download_path=download_path)
