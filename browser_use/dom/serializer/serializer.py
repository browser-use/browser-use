...
class DOMTreeSerializer:
	...
	def _assign_interactive_indices_and_mark_new_nodes(self, node: SimplifiedNode | None) -> None:
		...
		if not node.excluded_by_parent and not node.ignored_by_paint_order:
			...
			# Dropdown containers need to stay targetable even when their interactive
			# descendants are indexed separately. This covers ARIA menus, listboxes,
			# non-native combobox shells, and custom dropdown wrappers.
			attrs = node.original_node.attributes or {}
			...
			if is_dropdown_container and is_visible:
				# Always index visible dropdown containers - need to be targetable for
