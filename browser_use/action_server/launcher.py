...
def start_server_background(
	host: str = '127.0.0.1', port: int = 8766, debug: bool = False, wait_for_start: bool = True, max_wait_time: float = 10.0
	...
	try:
		...
		if wait_for_start:
			...
			while time.time() - start_time < max_wait_time:
				if is_server_running(host, port):
					print('✅ Browser Action Server started successfully!')
					return process
				time.sleep(0.5)
...
async def send_action(
	endpoint: str, data: dict[str, Any] | None = None, host: str = '127.0.0.1', port: int = 8766, timeout: float = 30.0
) -> dict[str, Any] | None:
	"""
...
async def type_text(selector: str, text: str, **kwargs) -> dict[str, Any] | None:
	...
	return await send_action(
		'type', {'selector': selector, 'text': text, 'timeout': timeout, **kwargs}, host=host, port=port, timeout=timeout
	)
...
async def scroll(direction: str, amount: int = 300, **kwargs) -> dict[str, Any] | None:
	...
	return await send_action(
		'scroll', {'direction': direction, 'amount': amount, 'timeout': timeout, **kwargs}, host=host, port=port, timeout=timeout
	)
...
async def wait_for_element(selector: str, timeout: float = 10.0, **kwargs) -> dict[str, Any] | None:
	...
	return await send_action(
		'wait',
		{'condition_type': 'element', 'selector': selector, 'timeout': timeout, **kwargs},
		host=host,
		port=port,
...
def ensure_server_running(host: str = '127.0.0.1', port: int = 8766, debug: bool = False, auto_start: bool = True) -> bool:
	"""
	...
	if auto_start:
		print('🚀 Starting Browser Action Server...')
