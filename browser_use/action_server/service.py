...
class BrowserActionServer:
	...
	async def start(self) -> None:
		...
		try:
			...
			config = uvicorn.Config(
				app=self.app, host=self.host, port=self.port, log_level='info' if self.debug else 'warning', access_log=self.debug
			)
		...
		async def handle_request(handler_func, request_data=None):
			...
			try:
				...
				error_detail = ErrorDetail(
					type=type(e).__name__, message=str(e), details={'context': 'request_handling'}, recoverable=True
				)

				return ErrorResponse(error=error_detail, execution_time_ms=execution_time)
	...
	async def _handle_navigate(self, request: NavigateRequest) -> ActionResponse:
		...
		return ActionResponse(
			data={'url': url, 'title': title, 'requested_url': request.url, 'wait_until': request.wait_until},
	...
	async def _handle_reload(self) -> ActionResponse:
		...
		return ActionResponse(data={'url': url, 'title': title, 'action': 'reload'}, message=f'Page reloaded: {title}')
	...
	async def _handle_back(self) -> ActionResponse:
		...
		return ActionResponse(data={'url': url, 'title': title, 'action': 'back'}, message=f'Went back to: {title}')
	...
	async def _handle_forward(self) -> ActionResponse:
		...
		return ActionResponse(data={'url': url, 'title': title, 'action': 'forward'}, message=f'Went forward to: {title}')
	...
	async def _handle_click(self, request: ClickRequest) -> ActionResponse:
		...
		click_options = {'button': request.button, 'click_count': request.click_count, 'timeout': request.timeout * 1000}

		if request.position:
			click_options['position'] = {'x': request.position[0], 'y': request.position[1]}  # type: ignore[assignment]
	...
	async def _handle_type(self, request: TypeRequest) -> ActionResponse:
		...
		current_value = (
			...
			if await element.evaluate('el => el.tagName === "INPUT" || el.tagName === "TEXTAREA"')
	...
	async def _handle_hover(self, request: HoverRequest) -> ActionResponse:
		...
		if request.position:
			hover_options['position'] = {'x': request.position[0], 'y': request.position[1]}  # type: ignore[assignment]

		await element.hover(**hover_options)

		return ActionResponse(
			data={'selector': request.selector, 'position': request.position}, message=f'Hovered over: {request.selector}'
		)
	...
	async def _handle_wait(self, request: WaitRequest) -> ActionResponse:
		...
		if request.condition_type == 'element':
			...
			return ActionResponse(
				data={'condition_type': 'element', 'selector': request.selector, 'visible': request.visible},
				message=f'Element found: {request.selector}',
			)

		elif request.condition_type == 'text':
			await page.wait_for_function(
				f'() => document.body.textContent.includes("{request.text}")', timeout=request.timeout * 1000
			)

			return ActionResponse(data={'condition_type': 'text', 'text': request.text}, message=f'Text found: {request.text}')

		elif request.condition_type == 'url':
			await page.wait_for_url(request.url, timeout=request.timeout * 1000)

			return ActionResponse(
				data={'condition_type': 'url', 'url': request.url, 'current_url': page.url}, message=f'URL matched: {request.url}'
			)
			...
			return ActionResponse(
				data={'condition_type': 'timeout', 'timeout': request.timeout}, message=f'Waited for {request.timeout} seconds'
			)
	...
	async def _handle_screenshot(self) -> ScreenshotResponse:
		...
		return ScreenshotResponse.create(
			screenshot_base64=screenshot_b64, format='png', size=(viewport['width'], viewport['height'])
		)
	...
	async def _handle_html(self) -> ActionResponse:
		...
		return ActionResponse(
			data={'html': html_content, 'length': len(html_content), 'url': page.url},
	...
	async def _handle_element_info(self, selector: str) -> ElementInfoResponse:
		...
		try:
			...
			if count == 0:
				return ElementInfoResponse.create(selector=selector, found=False)
			...
			return ElementInfoResponse.create(selector=selector, found=True, element_info=element_info)

		except Exception as e:
			return ElementInfoResponse.create(selector=selector, found=False, element_info={'error': str(e)})

	async def _handle_upload(self, request: UploadRequest) -> ActionResponse | ErrorResponse:
		...
		try:
			...
			if input_type != 'file':
				return ErrorResponse(
					error=ErrorDetail(
						...
						details={'selector': request.selector, 'actual_type': input_type},
						recoverable=False,
					)
				)
			...
			return ActionResponse(data=file_info, message=f'Uploaded file: {file_path.name}')
