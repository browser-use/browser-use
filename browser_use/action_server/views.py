...
class NavigateRequest(BaseRequest):
	...
	wait_until: Literal['load', 'domcontentloaded', 'networkidle'] = Field(
		default='domcontentloaded', description='When to consider navigation complete'
	)
...
class PageStatusResponse(BaseResponse):
	...
	def create(
		...
		data = {'url': url, 'title': title, 'loading': loading, 'ready_state': ready_state, 'element_count': element_count}
		...
		return cls(data=data, execution_time_ms=execution_time_ms)
...
class ScreenshotResponse(BaseResponse):
	...
	@classmethod
	def create(
		cls, screenshot_base64: str, format: str = 'png', size: tuple[int, int] | None = None, execution_time_ms: float = 0.0
	) -> ScreenshotResponse:
...
class ElementInfoResponse(BaseResponse):
	...
	@classmethod
	def create(
		cls, selector: str, found: bool, element_info: dict[str, Any] | None = None, execution_time_ms: float = 0.0
	) -> ElementInfoResponse:
		"""Create element info response"""
		data = {'selector': selector, 'found': found, 'element': element_info if found else None}
