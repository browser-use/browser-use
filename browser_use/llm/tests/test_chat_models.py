...
from browser_use.llm import ChatAnthropic, ChatGoogle, ChatGroq, ChatOpenAI, ChatOpenRouter, ChatOrcaRouter
...
class TestChatModels:
	...
	# OrcaRouter Tests
	@pytest.mark.asyncio
	async def test_orcarouter_ainvoke_normal(self):
		"""Test normal text response from OrcaRouter"""
		# Skip if no API key
		if not os.getenv('ORCAROUTER_API_KEY'):
			pytest.skip('ORCAROUTER_API_KEY not set')

		chat = ChatOrcaRouter(model='openai/gpt-4o-mini', api_key=os.getenv('ORCAROUTER_API_KEY'), temperature=0)
		response = await chat.ainvoke(self.CONVERSATION_MESSAGES)
		completion = response.completion
	...
	@pytest.mark.asyncio
	async def test_orcarouter_ainvoke_structured(self):
		"""Test structured output from OrcaRouter"""
		# Skip if no API key
		if not os.getenv('ORCAROUTER_API_KEY'):
			pytest.skip('ORCAROUTER_API_KEY not set')
