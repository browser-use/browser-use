		...
		if remove_min_items or remove_defaults:
			...
			def remove_forbidden_fields(obj: Any, *, in_properties: bool = False) -> None:
				...
				if isinstance(obj, dict):
					...
					if not in_properties and remove_min_items:
						obj.pop('minItems', None)
						obj.pop('min_items', None)
					if not in_properties and remove_defaults:
						obj.pop('default', None)
					# Recursively process all values
					for key, value in obj.items():
						if isinstance(value, (dict, list)):
							remove_forbidden_fields(value, in_properties=not in_properties and key == 'properties')
				elif isinstance(obj, list):
					for item in obj:
						if isinstance(item, (dict, list)):
							remove_forbidden_fields(item, in_properties=in_properties)
