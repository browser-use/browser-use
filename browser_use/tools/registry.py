...
from .mcp_protocol import MCPToolProtocol
...
class ToolRegistry:
    ...
    def register(cls, tool_name: str, tool_class: Any) -> None:
        ...
        if hasattr(tool_class, 'get_capabilities') and callable(tool_class.get_capabilities):
            pass
    ...
    @classmethod
    def get_tools_by_capability(cls, capability: str, context: Dict[str, Any]) -> List[str]:
        """
    ...
    def get_tool_metadata(cls, tool_name: str) -> Optional[Dict[str, Any]]:
        ...
        if hasattr(tool_class, 'metadata') and (
            callable(tool_class.metadata) or isinstance(tool_class.metadata, property)
        ):
            if isinstance(tool_class, type):
                tool = tool_class()
            else:
    ...
    def get_tool_examples(cls, tool_name: str) -> List[Dict[str, Any]]:
        ...
        if hasattr(tool_class, 'get_examples') and callable(tool_class.get_examples):
            if isinstance(tool_class, type):
                tool = tool_class()
            else:
                tool = tool_class
