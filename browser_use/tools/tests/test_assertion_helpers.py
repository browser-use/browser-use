...
def test_assert_url(mode, expected, actual, ok):
	...
	assert assert_url(summary, expected, mode) is ok


def test_assert_title_modes():
	summary = DummySummary(url='', title='Dashboard - Example')
	assert assert_title(summary, 'Dashboard - Example', 'equals')
	assert assert_title(summary, 'Dashboard', 'prefix')
	assert assert_title(summary, 'Example', 'contains')
	assert assert_title(summary, r'Dash.*Example', 'regex')
...
def test_is_visible_node():
	...
	assert is_visible_node(visible)
	assert not is_visible_node(hidden)
	assert not is_visible_node(zero_bounds)
