...
_DEFAULT_ACTION_TIMEOUT_S = _parse_env_action_timeout(
    os.getenv('BROWSER_USE_ACTION_TIMEOUT_S'))
...
class Tools(Generic[Context]):
    def __init__(
        ...
        self.registry = Registry[Context](
            exclude_actions if exclude_actions is not None else [])
        ...
        async def go_back(_: NoParamsAction, browser_session: BrowserSession):
            try:
                ...
                logger.error(
                    f'Failed to dispatch GoBackEvent: {type(e).__name__}: {e}')
        ...
        async def wait(seconds: int = 3):
            ...
            logger.info(
                f'🕒 waited for {seconds} second{"" if seconds == 1 else "s"}')
        ...
        async def _detect_new_tab_opened(
            ...
            try:
                ...
                new_tabs = [
                    t for t in tabs_after if t.target_id not in tabs_before]
                if new_tabs:
                    new_tab = new_tabs[0]
        ...
        async def _click_by_coordinate(params: ClickElementAction, browser_session: BrowserSession) -> ActionResult:
            ...
            try:
                ...
                event = browser_session.event_bus.dispatch(
                    ClickCoordinateEvent(
                        coordinate_x=actual_x, coordinate_y=actual_y, force=True)
                )
                await event
            ...
            try:
                ...
                if _is_autocomplete_field(node):
                    ...
                    if attrs.get('role') == 'combobox' or (attrs.get('aria-autocomplete', '') not in ('', 'none')):
                        # let JS dropdown populate before next action
                ...
                return ActionResult(
                    ...
                    metadata=input_metadata if isinstance(
                        input_metadata, dict) else None,
                )
            except BrowserError as e:
                return handle_browser_error(e)
            except Exception as e:
        ...
        async def upload_file(
                    ...
                    if browser_session.is_local and file_system and file_system.get_dir():
                        ...
                        if file_obj:
                            ...
                            file_system_path = str(
                                file_system.get_dir() / file_obj.full_name)
                        ...
                        if not browser_session.is_local:
                            ...
                            msg = f'File path {params.path} is not available. To fix: The user must add this file path to the available_file_paths parameter when creating the Agent. Example: Agent(task="...", llm=llm, browser=browser, available_file_paths=["{params.path}"])'
                            raise BrowserError(
                                message=msg, long_term_memory=msg)
            ...
            if file_input_node:
                create_task_with_error_handling(
                    browser_session.highlight_interaction_element(
                        file_input_node),
            ...
            if file_input_node is None:
                ...
                try:
                    ...
                    current_scroll_y = scroll_info.get(
                        'result', {}).get('value', 0)
                except Exception:
                    current_scroll_y = 0
                ...
                if closest_file_input:
                    ...
                    # Hail Mary fallback: search the global DOM for any file input
                    logger.info(
                        'No visible file upload element found in selector map, executing global DOM search fallback')
                    try:
                        js_code = """
						(() => {
							function findFileInput(doc) {
								...
								for (const iframe of iframes) {
									try {
										...
										const found = findFileInput(subDoc);
                        ...
                        js_result = await cdp_session.cdp_client.send.Runtime.evaluate(
                            params={'expression': js_code, 'returnByValue': False}, session_id=cdp_session.session_id
                        )
                        ...
                        if object_id:
                            ...
                            if backend_node_id:
                                ...
                                file_input_node = EnhancedDOMTreeNode(
                                    node_id=0,
                                ...
                                logger.info(
                                    f'Found fallback file input via global DOM search (backendNodeId: {backend_node_id})')
                    except Exception as e:
                        logger.debug(
                            f'Failed to locate fallback file input: {e}')
        ...
        async def close(params: CloseTabAction, browser_session: BrowserSession):
            ...
            try:
                ...
                logger.warning(
                    f'Tab {params.tab_id} may already be closed: {e}')
        ...
        async def extract(
            ...
            query = params['query'] if isinstance(
                params, dict) else params.query
            extract_links = params['extract_links'] if isinstance(
                params, dict) else params.extract_links
            extract_images = params.get('extract_images', False) if isinstance(
                params, dict) else params.extract_images
            start_from_char = params['start_from_char'] if isinstance(
                params, dict) else params.start_from_char
            output_schema: dict | None = params.get('output_schema') if isinstance(
                params, dict) else params.output_schema
            already_collected: list[str] = (
                params.get('already_collected', []) if isinstance(
                    params, dict) else params.already_collected
            )

            # Auto-enable extract_images if query contains image-related keywords
            _IMAGE_KEYWORDS = ['image', 'photo', 'picture', 'thumbnail',
                               'img url', 'image url', 'photo url', 'product image']
            ...
            if output_schema is not None:
                try:
                    ...
                    logger.warning(
                        f'Invalid output_schema, falling back to free-text extraction: {exc}')
                    output_schema = None
            ...
            chunks = chunk_markdown_by_structure(
                content, max_chunk_chars=MAX_CHAR_LIMIT, start_from_char=start_from_char)
            if not chunks:
                return ActionResult(
...
""".strip()
                ...
                if already_collected:
                    items_str = '\n'.join(
                        f'- {item}' for item in already_collected[:100])
                ...
                try:
                    ...
                    result_data: dict = response.completion.model_dump(
...
""".strip()
        ...
        async def search_page(params: SearchPageAction, browser_session: BrowserSession):
            ...
            if result.get('exceptionDetails'):
                error_text = result['exceptionDetails'].get(
        ...
        async def scroll(params: ScrollAction, browser_session: BrowserSession):
            try:
                ...
                try:
                    ...
                    viewport_height = int(css_viewport.get(
                        'clientHeight') or css_layout_viewport.get('clientHeight', 1000))

                    logger.debug(
                        f'Detected viewport height: {viewport_height}px')
                except Exception as e:
                    viewport_height = 1000  # Fallback to 1000px
                    logger.debug(
                        f'Failed to get viewport height, using fallback 1000px: {e}')
                ...
                if params.pages >= 1.0:
                    ...
                    for i in range(num_full_pages):
                        try:
                            ...
                            logger.warning(
                                f'Scroll {i + 1}/{num_full_pages} failed: {e}')
                    ...
                    event = browser_session.event_bus.dispatch(
                        ScrollEvent(
                            direction='down' if params.down else 'up', amount=pixels, node=node)
                    )
                    await event
                    await event.event_result(raise_if_any=True, raise_if_none=False)
                ...
                logger.error(
                    f'Failed to dispatch ScrollEvent: {type(e).__name__}: {e}')
        ...
        async def dropdown_options(params: GetDropdownOptionsAction, browser_session: BrowserSession):
            ...
            if not dropdown_data:
                raise ValueError(
                    'Failed to get dropdown options - no data returned')
        ...
        async def select_dropdown(params: SelectDropdownOptionAction, browser_session: BrowserSession):
            ...
            if not selection_data:
                raise ValueError(
                    'Failed to select dropdown option - no data returned')

            # Check if the selection was successful
            if selection_data.get('success') == 'true':
                # Extract the message from the returned data
                msg = selection_data.get(
                    'message', f'Selected option: {params.text}')
                return ActionResult(
                    extracted_content=msg,
        ...
        async def evaluate(code: str, browser_session: BrowserSession):
            ...
            try:
                ...
                result = await cdp_session.cdp_client.send.Runtime.evaluate(
                    params={'expression': validated_code,
                            'returnByValue': True, 'awaitPromise': True},
                    session_id=cdp_session.session_id,
                )
...
"""
                ...
                if value is None:
                    ...
                    result_text = str(
                        value) if 'value' in result_data else 'undefined'
                ...
                logger.debug(
                    f'JavaScript executed successfully, result length: {len(result_text)}')
    ...
    def _validate_and_fix_javascript(self, code: str) -> str:
        ...
        if changes_made:
            logger.debug(
                f'JavaScript fixes applied: {", ".join(changes_made)}')
    ...
    def _register_done_action(self, output_model: type[T] | None, display_files_in_done_text: bool = True):
        if output_model is not None:
            ...
            async def done(params: StructuredOutputAction, file_system: FileSystem, browser_session: BrowserSession):
                ...
                if params.files_to_display:
                    for file_name in params.files_to_display:
                        ...
                        if file_content:
                            attachments.append(
                                str(file_system.get_dir() / file_name))
                ...
                return ActionResult(
                    ...
                    extracted_content=json.dumps(
                        output_dict, ensure_ascii=False),
            ...
            async def done(params: DoneAction, file_system: FileSystem):
                ...
                attachments = [str(file_system.get_dir() / file_name)
    ...
    def set_coordinate_clicking(self, enabled: bool) -> None:
        ...
        self._register_click_action()
        logger.debug(
            f'Coordinate clicking {"enabled" if enabled else "disabled"}')
    ...
    def __getattr__(self, name: str):
        ...
        if name in self.registry.registry.actions:
            ...
            async def action_wrapper(**kwargs):
                ...
                action_params = {
                    k: v for k, v in kwargs.items() if k not in special_param_names}

                # Extract special params (injected dependencies) - exclude browser_session as it's positional
                special_kwargs = {k: v for k, v in kwargs.items(
                ) if k in special_param_names and k != 'browser_session'}
                ...
                DynamicActionModel = create_model(
                    ...
                    # type: ignore
                    **{name: (Union[action.param_model, None], None)},
                )
