...
def _log_format_message_line(message: BaseMessage, content: str, is_last_message: bool, terminal_width: int) -> list[str]:
	...
	try:
		...
		# Actual token counts are only available post-response via TokenCost service;
		# the ~ prefix signals this is an approximation, not a billed count.
		token_est = len(content) // 4
		token_str = f'~{token_est}'.rjust(5)
...
class MessageManager:
	...
	def _log_history_lines(self) -> str:
		...
		try:
			...
			return (
				f'📜 LLM Message history ({len(messages)} messages, ~{total_token_est} tokens):\n'
				+ '\n'.join(message_lines)
			)
		except Exception as e:
