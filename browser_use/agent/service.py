...
class Agent(Generic[Context, AgentStructuredOutput]):
	...
	def __init__(
		...
		**kwargs,
		...
		self.settings = AgentSettings(
			...
			auto_recovery_max_attempts=auto_recovery_max_attempts,
			auto_recovery_wait_seconds=auto_recovery_wait_seconds,
		...
		self._llm_service = LLMService(self, self._pipeline)
		self._llm_service.setup_fallback(fallback_llm)
		self._llm_service.verify_and_setup_llm()
	...
	def is_using_fallback_llm(self) -> bool:
		...
		return self._llm_service.is_using_fallback_llm

	@property
	def current_llm_model(self) -> str:
		"""Get the model name of the currently active LLM."""
		return self._llm_service.current_llm_model
	...
	async def _update_action_indices(
		...
		"""Update action indices based on current page state (delegates to action executor)."""
		return await self._pipeline.action_executor._update_action_indices(historical_element, action, browser_state_summary)
	...
	def _recursive_process_dict(dictionary: dict, url_replacements: dict[str, str]) -> None:
		...
		for k, v in dictionary.items():
			if isinstance(v, str):
				...
				LLMService._recursive_process_all_strings_inside_pydantic_model(v, url_replacements)
	...
	async def multi_act(self, actions: list[ActionModel]) -> list[ActionResult]:
		...
		"""
		return await self._pipeline.action_executor.multi_act(actions)

	async def log_completion(self) -> None:
		"""Log task completion."""
		await self._pipeline.action_executor.log_completion()
	...
	async def rerun_history(
		...
		"""
		return await self._pipeline.action_executor.rerun_history(
			history,
			max_retries=max_retries,
			skip_failures=skip_failures,
			delay_between_actions=delay_between_actions,
			max_step_interval=max_step_interval,
	...
	# ── LLMService forwarding wrappers ──────────────────────────

	async def get_model_output(self, input_messages: list[BaseMessage]) -> AgentOutput:
		"""Get next action from LLM based on current state (delegates to LLMService)."""
		return await self._llm_service.get_model_output(input_messages)

	async def _get_model_output_with_retry(self, input_messages: list[BaseMessage]) -> AgentOutput:
		"""Get model output with retry logic (delegates to LLMService)."""
		return await self._llm_service.get_model_output_with_retry(input_messages)
