...
class Agent(Generic[Context]):
    ...
    def __init__(
        ...
        # This will run in parallel with browser launch without leaving
        # dangling coroutines on unclean exits
    ...
    def _set_message_context(self) -> str | None:
        if self.tool_calling_method == 'raw':
            # For raw tool calling, only include actions with no filters
            # initially
    ...
    async def step(self, step_info: Optional[AgentStepInfo] = None) -> None:
        ...
        try:
            ...
            # If using raw tool calling method, we need to update the message
            # context with new actions
            if self.tool_calling_method == 'raw':
            ...
            try:
                ...
                # This is needed in case Ctrl+C was pressed during the
                # get_next_action call
                await self._raise_if_stopped_or_paused()
                ...
                if self.settings.save_conversation_path:
                    target = self.settings.save_conversation_path + \
                ...
                # check again if Ctrl+C was pressed before we commit the output
                # to history
                await self._raise_if_stopped_or_paused()
            ...
            self.telemetry.capture(
                AgentStepTelemetryEvent(
                    ...
                    step_error=[
                        r.error for r in result if r.error] if result else ['No result'],
                ))
            if not result:
                return
    ...
    async def get_next_action(
            self,
            input_messages: list[BaseMessage]) -> AgentOutput:
        """Get next action from LLM based on current state"""
        input_messages = self._convert_input_messages(input_messages)

        if self.tool_calling_method == 'raw':
            logger.debug(
                f'Using {self.tool_calling_method} for {self.chat_model_library}')
            try:
            ...
            # TODO: currently invoke does not return reasoning_content, we
            # should override invoke
            ...
            try:
                ...
                logger.warning(
                    f'Failed to parse model output: {output} {str(e)}')
        ...
        if not parsed:
            try:
                parsed_json = extract_json_from_model_output(
                    response['raw'].content)
                parsed = self.AgentOutput(**parsed_json)
            except Exception as e:
                logger.warning(
                    f'Failed to parse model output: {response["raw"].content} {str(e)}')
                raise ValueError('Could not parse response.')
        ...
        if not (
            hasattr(
                self.state,
                'paused') and (
                self.state.paused or self.state.stopped)):
            log_response(parsed)
    ...
    async def run(
        ...
        # Set up the Ctrl+C signal handler with callbacks specific to this
        # agent
        ...
        if hasattr(
                self,
                '_verification_task') and not self._verification_task.done():
            try:
        ...
        try:
            ...
            # Already handled by our signal handler, but catch any direct
            # KeyboardInterrupt as well
            logger.info(
    ...
    async def multi_act(
        ...
        for i, action in enumerate(actions):
            if action.get_index() is not None and i != 0:
                ...
                orig_target = cached_selector_map.get(
                    action.get_index())  # type: ignore
                orig_target_hash = orig_target.hash.branch_path_hash if orig_target else None
                new_target = new_selector_map.get(
                    action.get_index())  # type: ignore
                ...
                new_path_hashes = set(
                    e.hash.branch_path_hash for e in new_selector_map.values())
                if check_for_new_elements and not new_path_hashes.issubset(
                        cached_path_hashes):
                    # next action requires index but there are new elements on
                    # the page
            ...
            try:
                ...
                if results[-1].is_done or results[-1].error or i == len(
                        actions) - 1:
                    break

                await asyncio.sleep(self.browser_context.config.wait_between_actions)
                # hash all elements. if it is a subset of cached_state its fine
                # - else break (new elements on page)
    ...
    async def _validate_output(self) -> bool:
        ...
        if self.browser_context.session:
            ...
            msg = [
                SystemMessage(
                    content=system_msg), content.get_user_message(
    ...
    def _convert_initial_actions(
            self, actions: List[Dict[str, Dict[str, Any]]]) -> List[ActionModel]:
    ...
    async def _verify_llm_connection(self) -> bool:
        ...
        # Check if required environment variables are set for the model we're
        # using
        required_keys = REQUIRED_LLM_API_ENV_VARS.get(
            self.llm.__class__.__name__, [])
        if required_keys and not check_env_variables(
                required_keys, any_or_all=all):
        ...
        try:
            ...
            if test_answer in response_text:
                ...
                raise Exception(
                    'LLM responded to a simple test question incorrectly')
        except Exception as e:
            self.llm._verified_api_keys = False
            logger.error(
    ...
    async def _run_planner(self) -> Optional[str]:
        ...
        # No page = system prompt actions
        standard_actions = self.controller.registry.get_prompt_description()
        page_actions = self.controller.registry.get_prompt_description(
            page)  # Page-specific actions
        ...
        # Create planner message history using full message history with all
        # available actions
        planner_messages = [
    ...
    async def _update_action_models_for_page(self, page) -> None:
        ...
        self.DoneActionModel = self.controller.registry.create_action_model(
            include_actions=['done'], page=page)
        self.DoneAgentOutput = AgentOutput.type_with_custom_actions(
            self.DoneActionModel)
    ...
    def get_tool_capabilities(
            self, context: Dict[str, Any]) -> Dict[str, List[str]]:
        ...
        for tool_name, tool in self.tools.items():
            if hasattr(
                    tool, 'get_capabilities') and callable(
                ...
                try:
                    ...
                    logger.warning(
                        f"Error getting capabilities for tool {tool_name}: {e}")
    ...
    def get_tool_metadata(self, tool_name: str) -> Optional[Dict[str, Any]]:
        ...
        if hasattr(tool, 'metadata') and (
            ...
            try:
                ...
                logger.warning(
                    f"Error getting metadata for tool {tool_name}: {e}")
                return None
        ...
        if "method" in params:
            ...
            if hasattr(
                ...
                try:
                    ...
                    logger.error(
                        f"Error calling method {method_name} on tool {tool_name}: {e}")
                    return {
                        "success": False,
