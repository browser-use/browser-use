...
class Agent:
    def __init__(
        ...
        self.previous_model_output = None
    ...
    async def step(self, task_roadmap: Optional[str] = None ,step_info: Optional[AgentStepInfo] = None) -> None:
        ...
        try:
            ...
            self.message_manager.add_state_message(state, current_step_id, self._last_result, step_info, save_state=True)
            ...
            try:
                ...
                logger.info(f"Previous state message ID is {self.previous_step_id}")
                if self.previous_step_id:
                    logger.info(f"Deciding on previous state message with ID {self.previous_step_id} based on current model output.")
                    await self.message_manager._remove_last_state_message(self.previous_state, self.previous_step_id, model_output, task_roadmap)  # we dont want the whole state in the chat history

                self.previous_step_id = current_step_id
            ...
            if state:
                self._make_history_item(model_output, state, result, current_step_id)
    
    # def decide_on_previous_state(self, current_model_output):
    #     """Decide whether to keep or remove the state message from the previous step based on the current model output."""
    #     # Implement the logic here to decide based on `current_model_output` and `self.previous_model_output`
    #     # This is a stub for the logic implementation
    #     should_remove = self.message_manager.should_remove_previous_state(current_model_output)
    #     if should_remove:
    #         self.message_manager.remove_state_message_by_id(self.previous_step_id)
    #         logger.info(f"Previous state message with ID {self.previous_step_id} removed based on current model output.")
    ...
    # 		# Decide whether to save the state
    # 		save_state = False
    # 		if self.n_steps == 1 and state.url != "about:blank":  # First non-blank page
    # 			save_state = True
    # 		elif self.n_steps % 3 == 0:  # Every three steps
    # 			# Implement logic here to decide based on next action and current tab necessity
    # 			save_state = self.determine_save_state(model_output, state)

    # 		self.message_manager.add_state_message(state, self._last_result, step_info, save_state)
    # 		input_messages = self.message_manager.get_messages()
    ...
    # 			self._save_conversation(input_messages, model_output)
    # 			self.message_manager._remove_last_state_message()  # We don't want the whole state in the chat history
    ...
    def _make_history_item(
        ...
        state: BrowserState,
        result: list[ActionResult],
        step_id: str  # Adding step_id to trace the state with its creation step
    ) -> None:
        """Create and store history item with step identifier."""
        interacted_element = None
        len_result = len(result)
        ...
        state_history = BrowserStateHistory(
            ...
            interacted_element=interacted_elements,
            screenshot=state.screenshot,
            step_id=step_id  # Include step_id in the state history for traceability
        )
    ...
    async def run(self, max_steps: int = 100) -> AgentHistoryList:
        ...
        try:
            ...
            if self.initial_actions:
                task_roadmap = await self.create_task_roadmap(self.initial_actions)
    ...
    async def create_task_roadmap(task_description: str) -> str:
        """
        Create a detailed task roadmap.

        Args:
            task_description (str): User given task.

        Returns:
            str: A prompt that instructs the LLM to generate a detailed step-by-step roadmap.
        """
        prompt = f"Given the task: '{task_description}', generate a detailed step-by-step roadmap " \
                "that outlines all necessary actions to complete the task. Provide each step " \
                "with enough detail to guide an autonomous agent through the task execution. Number the steps and keep the output down to one sentence per step at max."
        
        response = client.chat.completions.create(
                    model="gpt-4o-mini",  # Adjust the model name as necessary
                    messages=[
                        {"role": "system", "content": "You are an task planner. Generate a detailed step-by-step roadmap for the given task."},
