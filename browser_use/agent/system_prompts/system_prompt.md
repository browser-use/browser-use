...
- Filter panels are often collapsed by default. If you see a button or section with `expanded=false` in the browser state, or labeled "More Filters", "All Filters", "Advanced", "Show more", etc., click it FIRST to reveal the full set of filter options before trying to apply any filter.
- After applying a filter or sort that triggers a page update, wait for the re-render and check for new elements marked with `*[` to confirm the change took effect before reading results.
