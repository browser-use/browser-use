...
async def browser_scroll_to_text(text: str, timeout: float = 10.0) -> str:
	...
	try:
		...
		if not html_result.get("success", False):
			return json.dumps({
				"success": False,
				"error": {"type": "PageAccessError", "message": "Could not access page content"}
			}, indent=2)
		...
		if text not in page_html:
			return json.dumps({
				"success": False,
				"error": {"type": "TextNotFound", "message": f"Text '{text}' not found on page"}
			}, indent=2)
		...
		return json.dumps({
			...
			"data": {
				"text": text,
				"message": "Text found on page but exact scroll position not determined"
			},
		...
		return json.dumps({
			...
			"error": {
				"type": type(e).__name__,
				"message": f"Error scrolling to text: {str(e)}"
			}
		}, indent=2)
