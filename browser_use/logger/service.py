...
class EventLogger:
	"""Append-only JSONL event logger for per-task action/element capture.

	Each agent action produces an `event_start` (with the captured element payload)
	and an `event_end` (with status). Downstream consumers (web-knowledge
	combine_logs / clean_logs) join workflow steps against these events.
	"""
	...
	def log_start(self, tool: str, action: dict | None = None, step_num: int | None = None) -> int:
		...
		self._write(
			{
				'event': 'event_start',
				'step': step_num if step_num is not None else self.step,
				'tool': tool,
				'action': action or {},
				'timestamp': time.time(),
			}
	...
	def close(self) -> None:
		...
		pass  # 'a' mode opens/closes per write; nothing to flush.
