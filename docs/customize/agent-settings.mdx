...
Browser Use includes anti-fingerprinting capabilities to help avoid detection by bot detection systems like Cloudflare and DataDome:
...
agent = Agent(
    task="Search for something on a site with bot detection",
    llm=llm,
...
When enabled, the anti-fingerprinting system:

- Overrides navigator properties to appear more like a regular browser
- Simulates realistic plugins and mime types
- Adds subtle noise to canvas, audio, and WebGL fingerprinting methods
- Normalizes hardware information like screen dimensions and color depth
- Ensures consistent behavior across all pages in a browser context

This is particularly useful for tasks that involve websites with strict bot detection mechanisms.
