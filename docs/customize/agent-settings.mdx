...
You can enable anti-fingerprinting capabilities to avoid bot detection mechanisms like those used by Cloudflare and DataDome:
...
When `anti_fingerprint` is enabled, the browser will use techniques to avoid detection by fingerprinting tools. This includes:
- Using Chrome as the browser channel
- Applying patches to modify browser behavior
- Preventing canvas and audio fingerprinting
- Modifying WebGL parameters to mimic a real device
