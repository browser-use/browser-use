...
Control how page content is extracted and processed.
...
extraction_config = BrowserExtractionConfig(
    # Enable/disable element extraction
    extract_elements=True,
    
    # Enable hybrid extraction with OmniParser
...
Browser Use integrates with Microsoft's OmniParser for enhanced UI element detection, particularly for challenging scenarios like CAPTCHAs and complex JavaScript UIs.

- **use_hybrid_extraction** (default: `False`)
  Enables the hybrid extraction approach that combines traditional DOM extraction with OmniParser's vision-based extraction.

- **omniparser** (default: `OmniParserSettings()`)
  Configuration for the OmniParser integration. See [OmniParser Integration](/omniparser_integration) for detailed documentation.

### Core Extraction Settings

- **extract_elements** (default: `True`)
  Enables or disables element extraction.
