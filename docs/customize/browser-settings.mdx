...
Directory to save video recordings. If a path is provided, a video of the session will be recorded to an MP4 file in this directory.
...
The frame size (width, height) of the video recording. If not set, it will default to the browser's viewport size. Example: `{"width": 1280, "height": 720}`

#### `record_video_framerate`

```python
record_video_framerate: int = 30
```

The framerate for the video recording. Defaults to 30 frames per second.
