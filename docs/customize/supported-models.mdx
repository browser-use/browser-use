...
We have recently switched from Langchain to our own implementation of the models. To migrate the previous code, just replace `from browser_use.llm import ChatOpenAI` with `from browser_use.llm import ChatOpenAI` etc. The methods should be compatible(ish).
