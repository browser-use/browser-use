...
- `close_after_run` (default: `True`): Whether to close the browser after the agent completes. Set to `False` when using `keep_alive=True` browsers or when manually managing browser lifecycle
- `on_step_start`: Hook function called at the beginning of each step. See [Lifecycle Hooks](/customize/hooks)
- `on_step_end`: Hook function called at the end of each step. See [Lifecycle Hooks](/customize/hooks)
