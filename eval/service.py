...
class TaskTracker:
    ...
    async def on_step_end(self, agent):
        ...
        self.save_results(None)  # Save progress after each step
    
    def save_results(self, final_agent_history):
...
async def run_agent_with_tracing(task: Task, browser: Browser | None = None, max_steps: int = 25):
    try:
        ...
        result = await agent.run(
            max_steps=max_steps,
            on_step_start=tracker.on_step_start,
            on_step_end=tracker.on_step_end
        )
        
        # Save final results
        final_results = tracker.save_results(result)
