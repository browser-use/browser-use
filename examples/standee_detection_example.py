...
async def naver_maps_standee_detection(
    ...
    if not task:
        ...
        2. Use the standee detection tool directly to analyze the photo:
           - Get the tool using: detector = agent.get_tool('standee_detection')
           - Process the photo using: result = detector.detect_from_url(photo_url)
           - Check the result for detections
        3. Log any photos with standee detections
        
        Remember to process each photo immediately after extracting its URL rather than collecting all URLs first.
        """
