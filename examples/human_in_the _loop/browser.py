	...
	async def _launch_novnc(self) -> None:
		...
		while attempt < max_attempts:
			if self.vnc_process.returncode is not None:
				stdout, stderr = await self.vnc_process.communicate()
				error_msg = f'noVNC process terminated unexpectedly:\n{stdout}\n{stderr}'
				raise RuntimeError(error_msg)

			if self._test_vnc_connection():
				return
		...
		if self.vnc_process.returncode is None:
			self.vnc_process.terminate()
			try:
				await asyncio.wait_for(self.vnc_process.wait(), timeout=2)
			except TimeoutError:
				self.vnc_process.kill()

		raise RuntimeError(error_msg)

	async def _cleanup_novnc(self) -> None:
		...
		try:
			...
			try:
				await asyncio.wait_for(self.vnc_process.wait(), timeout=3)
			except Exception:
