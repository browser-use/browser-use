...
| ⑤ 第三任务跨域验证 | 5/29 | 修复在非列表任务上是否成立 | github D⁺ 步数 -23%（趋势，n 小未显著）；三任务 D⁺ 均呈优于 C 趋势 |
...
| 补充 (06-01) | — | C×1 + D⁺×2 + D⁻×3 | 各组补至 n=5 | — | — | — | ✅×6（做 t-test 用） |

**全部 9 次跑次都找到了 issue #3912**（`browser-use Windows Issue - os.kill(pid, 0) - Fails on Windows`），结果稳定可验证。

### 结果（已补至 n=5，2026-06-01）

| 维度 | C (n=5) | D⁺ (n=5) | D⁻ (n=5 消融) |
|---|---|---|---|
| 成功率 | 5/5 | 5/5 | 5/5 |
| 步数 mean | 14.2 (±2.8) | **10.8 (±2.4)** | 11.0 (±2.3) |
| 耗时 mean | 451s (±133) | 314s (±84) | 300s (±41) |
| token mean | 294k (±70k) | 232k (±54k) | 214k (±20k) |

**Welch's t-test（双侧）**：

| 对比 | t | df | p | 判定 |
|---|---|---|---|---|
| 步数 C vs D⁺ | 2.08 | 7.8 | **0.072** | 边缘显著 |
| 耗时 C vs D⁺ | 1.94 | 6.8 | **0.095** | 边缘显著 |
| token C vs D⁺ | 1.57 | 7.5 | 0.158 | 不显著 |
| 步数 D⁺ vs D⁻ | -0.13 | 8.0 | 0.900 | 不显著（持平） |
| token D⁺ vs D⁻ | 0.67 | 5.1 | 0.532 | 不显著 |
...
> 📊 **统计更新（2026-06-01，已补 n=5）**：github 三组各补至 n=5 后，D⁺ vs C 步数 p 从 n=3 的 ≈0.15 改善到 **0.072（边缘显著）**、耗时 p=0.095；但严格按 α=0.05 **仍未跨线**。D⁺ vs D⁻ 步数 p=0.90（统计上**持平**）——先前 n=2 时"D⁻ 比 D⁺ 还省"的说法证实为高方差假象，已撤回。

### 关键结论（n=5 后，按可信度分层）

1. **D⁺ 优于 C：边缘显著，未达 α=0.05**。补样本后步数/耗时 p=0.072/0.095，方向稳定、效应一致（-24%/-30%），但严格不能宣称"统计显著"。诚实表述：**有边缘显著的优势趋势，需 n≥8 或配对设计确认**。

2. **D⁻ 与 D⁺：统计上持平（n=5 后定论）**。补样本后步数 10.8 vs 11.0（p=0.90）、token 232k vs 214k（p=0.53），**周期领航对最终步数/token 没有统计可见影响**。这印证贯穿三任务的核心发现：**navigator 的杠杆在开场 plan，不在周期领航**。⚠️ 注意是"持平"而非先前 n=2 误判的"D⁻ 更省"——那是高方差假象，已撤回。

3. **EFFICIENCY RULES 没有过度泛化**：D⁺ 在 github 上正确规划了 filter→sort→click 多步路径，没有 extract 误调用。这是**定性行为观察**，不依赖样本量，结论可靠。

4. **D⁺ 方差小于 C**：dur std 84 vs 133——方向在三任务上一致，作为趋势记录。

### 关于"周期 navigator 冗余"假设的最终判定（n=5）

先前 n=2 时观察到"D⁻ token 低于 D⁺"，曾推测"周期领航冗余"。**补到 n=5 后证伪了这个具体说法**：D⁺ 与 D⁻ 在步数（p=0.90）和 token（p=0.53）上**统计持平**，不存在"D⁻ 更省"。

→ 修正后的准确结论：**周期领航既不显著帮忙、也不显著拖累**——它对最终结果的净影响在统计上不可见。这仍然支持"navigator 价值集中在开场 plan"的主线，但**不能**说"周期领航是冗余应该删掉"（删掉它结果不变，保留它也不变，需要看它在更难任务/失败恢复场景下是否有价值才能定）。

### 项目贡献（措辞校准，n=5 后）

✅ **navigator 增益的负面反转被修复**：paper 任务 D 从输 C 50% 反转为优 C 38%（稳健）。
✅ **EFFICIENCY RULES 鲁棒性**：三任务上无过度泛化，行为路径正确（定性可靠）。
✅ **github n=5 + Welch t-test**：D⁺ vs C 步数 p=0.072 / 耗时 p=0.095（边缘显著）；D⁺ vs D⁻ p=0.90（持平）。
⚠️ **三任务交叉趋势**：21/38/24% 步数节省，paper 稳健、github 边缘显著、hospital 待检验（n=3）。
✅ **"周期领航冗余"假设被证伪**：n=5 后 D⁺≈D⁻（非 D⁻ 更省），修正为"周期领航净影响统计不可见"。
...
[Day 3] 验证：D 在 paper 上步数 17→7, token -57%, 反过来比 C 还快 38%
   ↓
[Day 4] 扩第 3 任务 github → D⁺ mean 步数 -23%（趋势，n 小未显著）；三任务 D⁺ 均呈优于 C 趋势
   ↓
[Day 4] 待验证假设：D⁻ 在 github 上 token 看似 < D⁺，但 n=2 方差过大，不能定论（已撤回"周期 navigator 冗余"结论）
   ↓
[Day 6] 复盘统计严谨性：github -23% 经 t-test p≈0.15 不显著 → 文档措辞降级为"趋势"，计划补 n=5
   ↓
[当前] 已 commit + push 修复与文档；下一步补 github 样本做显著性检验
```
...
| 6 | 跨域增益趋势 | 阶段 ⑤ | 三类任务上 D⁺ mean 步数均低于 C（21/38/23%），其中 paper 稳健、hospital/github 为趋势（n 小，待 n=5 统计确认） |
