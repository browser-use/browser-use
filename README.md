...
<p align="center">
  <a href="https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=browser-use">
    <img src="./static/atlas-cloud-logo.png" alt="Atlas Cloud" width="200">
  </a>
</p>

> 🎁 **[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=browser-use)** is a full-modal, OpenAI-compatible AI inference platform. Plug it into Browser Use as a drop-in `llm` backend via `ChatAtlasCloud` to drive your agent with DeepSeek, Qwen, GLM, Kimi, MiniMax and more — one API key, no multi-vendor setup. Budget-friendly: [coding plan](https://www.atlascloud.ai/console/coding-plan).

```python
from browser_use import Agent, ChatAtlasCloud

agent = Agent(
    task="Find the price of the latest GPU on a shopping site",
    llm=ChatAtlasCloud(model="deepseek-ai/deepseek-v4-pro"),  # reads ATLASCLOUD_API_KEY
)
```

`deepseek-ai/deepseek-v4-pro` is a reasoning model — keep `max_completion_tokens` generous (the provider defaults to `4096`).
