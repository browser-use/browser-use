...
def test_status_dict(llm, requested, expected_method, tool_ok, multi_ok):
	...
	assert status.get('selected_tool_calling_method') == expected_method
	assert status.get('supports_tool_calling_method') is tool_ok
	assert status.get('passed_iq_test') is True  # all mocks answer Paris
	assert status.get('supports_multiple_human_msgs') is multi_ok
