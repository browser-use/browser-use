...
async def test_cdp_client_headers_passed_main():
    ...
    session = BrowserSession(
        cdp_url="wss://bedrock-agentcore.us-east-1.amazonaws.com/browser-streams/test",
...
async def test_cdp_session_for_target_headers_passed():
    ...
    with patch('browser_use.browser.session.CDPClient') as mock_cdp_client_class:
        ...
        mock_new_client.send.Target.getTargetInfo = AsyncMock(return_value={
            'targetInfo': {'targetId': 'test-target', 'title': 'Test', 'url': 'about:blank'}
        })
