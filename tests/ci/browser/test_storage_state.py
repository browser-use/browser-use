...
async def test_storage_state_origin_scoping():
	...
	watchdog = StorageStateWatchdog.model_construct(browser_session=mock_session, event_bus=MagicMock(), _logger=MagicMock())

	# Test data with BOTH cookies and localStorage
	test_storage_data = {
		'cookies': [{'name': 'session', 'value': 'val', 'domain': '.example.com', 'path': '/'}],
		'origins': [
			{
	...
	with patch('anyio.Path.read_text', new_callable=AsyncMock) as mock_read, patch('os.path.exists', return_value=True):
