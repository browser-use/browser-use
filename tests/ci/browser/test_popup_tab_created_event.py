...
async def test_original_page_functional_after_popup(popup_browser_session, popup_base_url):
	...
	result = await cdp_session.cdp_client.send.Runtime.evaluate(
		params={
			'expression': "(() => { const status = document.getElementById('status'); status.textContent = 'still-responsive'; return status.textContent; })()",
			'returnByValue': True,
		},
		session_id=cdp_session.session_id,
	)

	assert result.get('result', {}).get('value') == 'still-responsive', (
